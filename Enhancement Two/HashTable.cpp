...
unsigned int HashTable::hash(int key) {
    ...
    // Using a modulo hash to return key tableSize
    return key % tableSize;
}
...
void HashTable::Insert(Bid bid) {
   ...
   //Temp key
    ...
    //checks to see if node is there then will overwrite default construct
    ...
    else { //this will bring us to the bottom of the list

        //creates a node as an iterator and points to hash location
        Node* newNode = new Node(bid, tempKey);

        //check list to find last location
...
void HashTable::Remove(string bidId) {
    ...
    //make currnode the head node
    Node* cursor = &nodes.at(hash(stoi(bidId)));
    Node* tempNode = NULL;

    //check for chain, take methods if there is chain if not check head node

    if (cursor->bid.bidId == bidId) {

        //tempnode to next node, copy +1 to head node and delete +1 node
        ...
        while (cursor != NULL && cursor->bid.bidId != bidId) {
            ...
            tempNode = cursor; //temp becomes cursor
            cursor = cursor->next; // cursor moves up
        }

        if (cursor == NULL) { //scan list and key not found
   ...
   if (cursor->bid.bidId == bidId) { // matches head with no chain
   ...
   else { // no match or chain at the head
        cout << "This bid does not exist." endl;
   }
 }
